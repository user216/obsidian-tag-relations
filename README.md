...
Beside the plex is a **starting-point column** listing every tag, pinned and bookmarked first. A plex only ever shows one neighbourhood, which makes it good for walking and poor for arriving — this is the way in. Collapse it from its own heading, or turn it off in Settings → Views → Plex — where you can also put it on the **left or the right**. Which side reads best depends on where the view is docked: in a right sidebar the list usually wants to be against the plex rather than the window edge.
