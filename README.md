...
Beside the plex is a **starting-point column** listing every tag, pinned and bookmarked first. A plex only ever shows one neighbourhood, which makes it good for walking and poor for arriving — this is the way in. Two buttons on its heading switch it between a **flat list** and one **grouped under each main-tag** — two different ways of finding a tag you can't quite name: by the word, or by remembering what it sits under. Group sections fold on the same state as the Groups view, so a group folded in one is folded everywhere.

Collapse the whole column from its own heading, or turn it off in Settings → Views → Plex — where you can also put it on the **left or the right**. Which side reads best depends on where the view is docked: in a right sidebar the list usually wants to be against the plex rather than the window edge.
